...
## Database day-to-day
Reset: `docker compose -f infra/docker-compose.dev.yml down -v && up -d && migrate && seed`. **If a backend dev server (`uv run uvicorn`) is already running, restart it too** - its connection pool holds dead sockets to the destroyed Postgres container after `down -v` and doesn't always detect this on checkout, producing intermittent `connection is closed` 500s that look exactly like flaky e2e (see docs/MEMORY.md). Inspect: `docker compose -f infra/docker-compose.dev.yml exec postgres psql -U app appdb`. New table/column: edit docs/DATABASE.md -> write db/migrations/000N_*.sql -> migrate locally -> CI migration job proves it -> CD applies to prod. Never edit an applied migration; never point local tools at the prod DATABASE_URL.
