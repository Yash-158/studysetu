...
async def test_claim1_cache_key_is_provider_agnostic_and_that_matches_S13_design_intent(monkeypatch):
    ...
    reason claude_sonnet/gemini_flash/groq_llama are listed as one ordered fallback chain per
    ...
    async def _claude_ok(model_cfg, *, system, prompt):
        claude_calls["n"] += 1
        return _ok('{"from": "claude", "call": 1}')

    async def _gemini_would_answer_differently(model_cfg, *, system, prompt):
        gemini_calls["n"] += 1
        return _ok('{"from": "gemini", "call": 2}')

    monkeypatch.setattr(claude_provider, "invoke", _claude_ok)
    ...
    assert r1.content == {"from": "claude", "call": 1}
    ...
    # even reached.
    async def _claude_now_unreachable(model_cfg, *, system, prompt):
        raise ProviderError("claude unreachable for call 2 - only gemini COULD answer now")

    monkeypatch.setattr(claude_provider, "invoke", _claude_now_unreachable)
    ...
    assert r2.content == {"from": "claude", "call": 1}, "cached content must be served as-is, not regenerated by a different provider"
...
async def test_claim2a_malformed_json_retries_same_provider_never_reaches_next_in_chain(monkeypatch, capsys):
    ...
    async def _claude_bad_then_good(model_cfg, *, system, prompt):
        calls["claude"] += 1
        return _ok("this is not valid json at all {{{") if calls["claude"] == 1 else _ok('{"recovered": true}')
    ...
    assert calls["claude"] == 2, "malformed JSON must trigger exactly one same-provider retry"
    ...
    assert [r.provider for r in rows] == ["claude_sonnet", "claude_sonnet"]
...
async def test_claim2b_hard_failure_fails_over_immediately_with_no_retry(monkeypatch, capsys):
    ...
    async def _claude_hard_failure(model_cfg, *, system, prompt):
        calls["claude"] += 1
    ...
    monkeypatch.setattr(claude_provider, "invoke", _claude_hard_failure)
    ...
    assert calls["claude"] == 1, "a hard failure must NOT be retried on the same provider"
    ...
    for r in rows:
        print(f"  provider={r.provider!r} success={r.success} cache_hit={r.cache_hit} error={r.error!r}")
    assert [r.provider for r in rows] == ["claude_sonnet", "gemini_flash"]
    assert [r.success for r in rows] == [False, True]
